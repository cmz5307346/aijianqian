...
public class YYCircleindicator extends View {

    private int radius = 4;
    private int selectradius = 6;
    ...
    public YYCircleindicator(Context context) {
        super(context);
    ...
    public YYCircleindicator(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        try {
            ...
            radius = (int) a.getDimension(R.styleable.XCircleIndicator_radius,
                    radius);
            selectradius = (int) a.getDimension(R.styleable.XCircleIndicator_selectradius, radius);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int iLoop = 0; iLoop < pageTotalCount; iLoop++) {
            canvas.drawCircle(getPaddingLeft() + selectradius
                    + (iLoop * (2 * selectradius + circleInterval)), getPaddingTop()
                    + selectradius, radius, mPaintStroke);
        }
        int cx = 0;
        cx = (2 * selectradius + circleInterval) * currentPage;
        canvas.drawCircle(getPaddingLeft() + selectradius + cx, getPaddingTop()
                + selectradius, selectradius, mPaintFill);
    }
    ...
    private int measureWidth(int measureSpec) {
        ...
        if (specMode == MeasureSpec.EXACTLY) {
            ...
            result = getPaddingLeft() + getPaddingRight()
                    + (pageTotalCount * 2 * selectradius) + (pageTotalCount - 1)
                    * circleInterval;
    ...
    private int measureHeight(int measureSpec) {
        ...
        // Measure the height
            ...
            result = 2 * selectradius + getPaddingTop() + getPaddingBottom();
            // Respect AT_MOST value if that was what is called for by
            // measureSpec
            if (specMode == MeasureSpec.AT_MOST) {
                result = Math.min(result, specSize);
